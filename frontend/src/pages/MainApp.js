...
export default function MainApp() {
  ...
  const filtered = conversations.filter(c => {
    const name = (c.displayName || c.name || c.username || '').toLowerCase();
    return name.includes(search.toLowerCase());
  });

  // Mobile: when in chat, hide bottom nav and use full height
      ...
      <div className="conversation-list">
        {filtered.length === 0 ? (
          <div className="no-chats">
            <p>No chats yet</p>
            <small>Go to Space to find people to chat</small>
          </div>
        ) : (
          filtered.map(conv => {
            ...
            return (
              <div key={conv.id} className={`conversation-item ${activeChat?.id === conv.id ? 'active' : ''}`} onClick={() => openChat(conv)}>
                <div className="conv-avatar-wrap">
                  {src ? <img src={src} alt="" className="conv-avatar" /> : (
  ...
  const renderContent = () => {
    ...
    if (tab === 'refer') return (
      <div className="main-content full refer-page">
        <div className="refer-header"><h2>Refer & Earn</h2><p>Invite friends, grow your reach</p></div>
        <div className="refer-card">
          ...
          <button className="refer-copy-btn" onClick={() => {
            navigator.clipboard.writeText(`Join me on Revilla! Code: ${currentUser?.referralCode} — revilla.vercel.app`);
            alert('Copied!');
