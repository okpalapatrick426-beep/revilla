...
export default function ChatWindow({ conversation, currentUser, onBack, isOnline, onQueue, onQueueUpdate }) {
  ...
  useEffect(() => {
    ...
    // Real-time blue ticks — when recipient opens chat, all our sent msgs turn blue
    const onMessagesRead = ({ messageIds, readBy }) => {
      if (readBy !== conversation.id) return;
      setMessages(prev => prev.map(m =>
        messageIds.includes(m.id) ? { ...m, readBy: [...(m.readBy||[]), readBy] } : m
      ));
    };
...
const s = {
  ...
  voicePlayer: { display:'flex',alignItems:'center',gap:8,background:'rgba(255,255,255,0.06)',borderRadius:12,padding:'8px 10px',minWidth:180,maxWidth:240 },
